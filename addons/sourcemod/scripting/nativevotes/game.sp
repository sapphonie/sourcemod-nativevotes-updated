...
static void TF2CSGO_UpdateVoteCounts(ArrayList votes)
{
    if (CheckVoteController())
    {
        for (int i, size = votes.Length; i < size; ++i)
        {
...
static void TF2CSGO_DisplayVote(NativeVote vote, int[] clients, int num_clients)
{
    ...
    if (bNoVoteButton)
    {
        ...
        FormatEx(display, sizeof display, "%T", "No Vote", LANG_SERVER);
    ...
    if (bYesNo)
    {
        ...
        {
            char option[8];
            FormatEx(option, sizeof option, "%s%d", TF2CSGO_VOTE_PREFIX, i + 1);
...
static void TF2CSGO_SendOptionsToClient(NativeVote vote, int client)
{
    ...
    if (bNoVoteButton)
    {
        start = 1;
        char option[8];
        FormatEx(option, sizeof option, "%s1", TF2CSGO_VOTE_PREFIX);
        
        char display[TRANSLATION_LENGTH];
        FormatEx(display, sizeof display, "%T", "No Vote", client);
        optionsEvent.SetString(option, display);
    }
